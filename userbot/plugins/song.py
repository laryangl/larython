...
#                           STRINGS                           #
# =========================================================== #
SONG_SEARCH_STRING = "<code>wi8 ..!  جاري البحث </code>"
...
async def _(event):
    ...
    catevent = await edit_or_reply(event, "`wi8 ..!  جاري البحث `")
    ...
    if not os.path.exists(song_file):
        return await catevent.edit(
            f"آسف!.  لا يمكنني العثور على أي فيديو / صوت ذي صلة بـ `{query}`"
        )
...
@catub.cat_cmd(
    ...
    info={
        "header": "للحصول على أغاني الفيديو من يوتيوب.",
        "description": "يبحث هذا الأمر بشكل أساسي في youtube ويرسل الفيديو الأول",
